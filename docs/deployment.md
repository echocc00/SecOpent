...
- [ ] 适配器镜像 digest-pin（§8.1，M5）：`infrastructure/adapters/image_catalog.py` 为 subfinder/httpx/nuclei/nmap/dalfox 等已 pin 真 sha256（`docker images --digests` 复核）；`fingerprinthub`/`restler`/`schemathesis`/`zap`/`prowler`/`trivy`/`kube_bench`/`checkov`/`scoutsuite` digest 仍空（TBD）。各 adapter manifest 的 `upstream.digest` 当前为占位符（`sha256:<adapter>-<ver>`），M5 容器构建时须对齐到 image_catalog 真 digest，否则 `base.py` 的 `docker run <manifest.digest>` 用占位符会失败
